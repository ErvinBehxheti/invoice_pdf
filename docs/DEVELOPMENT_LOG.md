...
**One cosmetic gap found, not fixed here:** the Clerk-hosted sign-in modal reads "Sign in to **InvoicePDF**" while the product is branded "InvoiceFlow" everywhere else (sidebar, README, marketing pages). That name lives in the Clerk dashboard's application settings, not in this codebase, so it's a one-time manual rename in Clerk's dashboard rather than a code change — flagged for whoever does the pre-launch pass. The Stripe test-mode account has the same gap ("Invoice PDF sandbox" on the Checkout page, see below) — same fix, same place: dashboard settings, not code.

<a id="stripe-test-mode"></a>**Stripe test-mode checkout, wired and verified live.** `STRIPE_SECRET_KEY` and the publishable key were already real test keys; the missing piece was `STRIPE_PRICE_ID_PRO_EUR`, still a placeholder. Created the actual product ("InvoiceFlow Pro") and a recurring €2.00/month EUR price directly via the Stripe API (no `stripe login` needed for this part — only `stripe listen`, for local webhook forwarding, requires the interactive CLI login). Then drove the real flow with a throwaway Clerk test user: `/settings/billing` → "Upgrade to Pro" → landed on an actual Stripe-hosted Checkout page ("Subscribe to InvoiceFlow Pro", €2.00/month, correct description) → filled Stripe's standard test card (`4242 4242 4242 4242`) → submitted → redirected back to the app successfully.

That confirmed the checkout mechanics work end-to-end, but also confirmed the one piece that's still genuinely open: without `stripe listen` forwarding events to localhost (or a deployed webhook endpoint), the app never receives `checkout.session.completed`, so `planTier` stays `free` even after a successful test subscription. This isn't a bug — it's the expected gap the docs already called out — but it's now something actually observed live rather than just reasoned about. Confirmed via the Stripe API afterward that all 3 test subscriptions created during this pass did go through correctly on Stripe's side, then canceled those subscriptions and deleted the test customers/Clerk user, leaving no test data behind.

**Security hygiene pass, before this repo is public.** Swept the full git history (`git log --all -p`) for anything secret-shaped (`sk_`, `pk_`, `whsec_`, Resend keys, Postgres connection strings, Vercel Blob tokens) — nothing real turned up, only placeholder strings already in the docs and `.env.local.example`. Confirmed `.env.local` itself has never been tracked. Rotated `CRON_SECRET` from a weak, reused-looking value to a random 32-byte hex string, and re-verified the cron route live: no auth → 401, wrong secret → 401, correct new secret → 200.

**The webhook gap, actually closed.** Assumed `stripe listen` needed the full interactive `stripe login` OAuth flow — it doesn't. It accepts `--api-key` directly, and the secret key was already sitting in `.env.local`. Ran `stripe listen --api-key $STRIPE_SECRET_KEY --forward-to localhost:3000/api/webhooks/stripe`, got a real webhook signing secret with zero browser interaction, and wired it in. Then drove a full checkout with a fresh test user while `stripe listen` forwarded events live: `checkout.session.completed` arrived, the handler returned `200`, and a direct DB check confirmed `planTier: "pro"`, `subscriptionStatus: "active"` — the first time this project has verified the complete signup → pay → webhook → upgrade loop for real, rather than via self-signed test events or checkout mechanics alone.

**One genuine, previously-undiscovered bug, found only because there was finally a real Pro user to look at:** the sidebar's plan-status card and the "Clients" nav badge were both hardcoded. `Sidebar.tsx` never received or checked `planTier` at all — it unconditionally rendered "Free plan / Upgrade to Pro" and a "Pro" badge on the Clients link, regardless of the signed-in user's actual plan. Every paying user would have seen a permanent upsell for a plan they already had, and a "this needs Pro" badge on a feature they'd already unlocked. Nobody had caught this before because no Stripe webhook had ever actually fired against a real signed-in session until this pass. Fixed by having `app/(app)/layout.tsx` fetch the user once via the existing `getOrCreateUser()` and pass `isPro` down through `AppShell` into `Sidebar`, which now shows a real "⚡ Pro plan" card and hides the Clients badge once unlocked. Verified live: the same Pro test user that had just completed checkout now saw the corrected sidebar, confirmed via screenshot before cleanup.

One real gap found in the process: `.gitignore`'s `.env*` pattern was blanket-excluding `.env.local.example` along with the real `.env.local` — so the template file this README tells new readers to `cp` had never actually been committed. Added a `!.env.local.example` exception; the file itself only ever held placeholder values, confirmed before un-ignoring it.

## Post-Deploy Bug Reports

The project went live on Vercel, reusing the same Neon database as local dev. Two real bugs turned up almost immediately from actual usage — the kind that only surface once a project is in front of a real (even if small) audience instead of a single developer's own testing.

**Bug: "Save invoice" while signed out silently dropped the invoice after sign-in.** Root cause: `InvoiceBuilder`'s auto-save-after-sign-in logic detected the signed-out→signed-in transition with a plain `useRef` flag compared across renders. That works for email/password sign-in inside Clerk's modal (a pure client-side state change, no navigation) — but breaks for OAuth sign-in (Google/Apple), which requires actually leaving the page for the provider's login screen and coming back. That's a full browser navigation: the whole React tree unmounts and remounts fresh, resetting every ref to its initial value. On the fresh mount, `isSignedIn` is already `true` from the very first render, so there's no "transition" left to detect — the ref-based check just recorded the current state and moved on, and the draft sat in `localStorage` forever, never POSTed. This was never caught locally because every earlier verification pass in this project used Clerk's backend `signInTokens` API to sign in test users directly, which is itself a full-page navigation to a ticket URL — it never actually exercised the client-side `<SignInButton mode="modal">` + modal-close-without-reload path that a real signed-in-via-email-password user hits. Real user testing (an actual person clicking actual buttons) is what caught it.

Fixed by replacing the ref-based transition detection with a `localStorage` flag (`invoiceflow_pending_save`), set the moment the signed-out "Save invoice" button is clicked (confirmed via reading Clerk's own `SignInButton` source that it preserves and calls a child's `onClick` before opening the modal), and checked on mount whenever the app is hydrated and signed in — a check that survives a full-page reload because `localStorage`, unlike React state, isn't wiped by one. Verified by scripting the exact failure mode: filled the form anonymously, clicked Save, then did a full top-level page navigation to complete sign-in (rather than a same-page ticket redirect) to genuinely simulate the OAuth round-trip, then reloaded `/invoices/new` fresh and confirmed the pending save fired and the invoice landed on the dashboard.

That fix itself introduced a second bug, caught by React's own development-mode double-effect-invocation (Strict Mode) rather than by me: the first version created **two** identical invoices instead of one, because the effect wasn't safe to run twice. Added a `useRef` guard alongside the `localStorage` flag so a second invocation within the same mount is a no-op, while a genuine fresh remount (the real OAuth-reload case) still resets the ref and works correctly. Re-verified the same scripted scenario afterward: exactly one invoice created.

**Bug: Chrome/Google flags the post-payment redirect as a suspected credit-card-stealing site.** Reported after a real Stripe test-mode payment — Chrome's Safe Browsing blocked navigation back to the app with a phishing-style warning. See the README/next-steps notes for the investigation and mitigation, since this is an external reputation issue (about the deployed domain, not the app's code) rather than something fixable by editing a file in this repo.
